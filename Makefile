...
# Enable 24 parallel jobs by default
...
release: prepare $(REAL_LIB_RELEASE)

debug: prepare $(REAL_LIB_DEBUG)
...
$(SONAME): $(REAL_LIB_DEBUG) $(REAL_LIB_RELEASE)
	ln -sf $(REAL_LIB_DEBUG) $@
...
test: $(SONAME)
	...
	$(call CHECK_AND_UPLOAD,./test/test_app);
	$(call CHECK_AND_UPLOAD,./$(SONAME));
	$(call CHECK_AND_UPLOAD,./$(REAL_LIB_RELEASE));
	$(call CHECK_AND_UPLOAD,./$(REAL_LIB_DEBUG));
...
mk:
	$(call CHECK_AND_UPLOAD,./1_1.img);
	$(call CHECK_AND_UPLOAD,./FlashDeviceParameters.txt);
