...
typedef struct
{
    uint32_t roi_width;     //roi有效宽度值
    uint32_t sensor_width;  //sensor出图宽度信息
    uint32_t soc_width;     //soc出图实际宽度
}SENSOR_SOC_WIDTH;
...
public:
    ...
    ~DEV_SENSOR();
public:
    DEV_RTN dev_sensor_init(uint8_t, DEV_SENSOR_TYPE, uint8_t, uint8_t, uint8_t,void*);
...
private:
    ...
    DEV_RTN m_sensor_stream_set(bool);
